...
XPC 接口、状态模型和 SwiftUI 展示层均不需要修改。修复后无论 helper 在电池还是
插电状态下启动，都会公开相同的设备能源模式能力；当前作用域仍由现有电池状态决定。

由于解析发生在常驻 helper 进程中，同时将 helper 修订号递增。新版 App 如果连接到
修复前已经运行的 helper，会将其识别为需要修复，而不是继续接受旧进程返回的空能力
集合；用户通过现有修复入口重新加载 helper 后，新解析逻辑立即生效。
