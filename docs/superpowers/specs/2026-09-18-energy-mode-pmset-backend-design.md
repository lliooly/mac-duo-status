...
将 `PowerControlView` 调整为接近截图的原生模式列表：

- 顶部显示当前供电来源：电池或电源适配器。
- 模式列表只编辑当前实际供电来源对应的配置；不提供手动切换作用域的控件。
- 面板打开期间发生供电来源变化时，模式列表跟随实际供电来源切换；不在插拔事件中主动写入模式。
...
- `pmset` 能力缺失时，主应用仍保持 Battery、Network 和 System Health 的只读
  能力。
- 不自动修改两个供电来源；当前供电来源决定只写入对应的 `-b` 或 `-c`。
...
3. 插拔电源后，Duo Status 的当前模式和供电来源同步变化。
   面板保持打开时，模式列表也必须跟随当前供电来源变化。
