...
项目已完成 V1 基础状态能力和能源模式后端的主要实现。菜单栏入口、弹出面板、设置面板、统一状态源、真实系统状态采集和电源辅助进程均已接入；Wi-Fi 只读展示由应用提供，连接和切换通过系统 Wi-Fi 设置完成；仍需在目标 Apple silicon 设备上完成硬件场景验收。

产品定位是个人使用、轻量和稳定优先：

- 读取当前 Mac 的本机状态，并在能力可用时执行用户主动发起的电源本地控制。
- 优先使用 Apple 公开 API。
- 不连接服务器，不同步账号数据。
...
- [移除 Wi-Fi 联网控制设计](docs/superpowers/specs/2026-09-20-remove-wifi-networking-design.md)
- [移除 Wi-Fi 联网控制实现计划](docs/superpowers/plans/2026-09-20-remove-wifi-networking-plan.md)
...
当前发布准备以 Apple silicon 的无签名 zip 包为目标。无签名包可以用于个人测试或手动分发，但首次打开可能受到 macOS 安全提示影响；电源控制依赖签名且获授权的 helper，未满足条件时应用会保持只读降级。完整流程见[发布说明](docs/releasing.md)。

## 隐私和权限

应用只读取和展示当前 Mac 的本机状态；电源控制也只作用于当前 Mac。Wi-Fi 连接由系统 Wi-Fi 设置处理，应用不读取或保存 Wi-Fi 密码。主应用不以 root 运行，不保存管理员密码或临时授权凭证；辅助进程仅在明确签名和授权后，使用固定参数调用系统内置 `/usr/bin/pmset`。
