...
项目已完成 V1 基础状态能力与合并控制版本的主要实现。菜单栏入口、弹出面板、设置面板、统一状态源、真实系统状态采集、Wi-Fi 控制链路和电源辅助进程合同均已接入；仍需在目标 Apple silicon 设备上完成硬件场景验收。
...
- 低电量模式读取、电源能力展示和辅助进程授权状态展示。
- 电源模式/充电上限控制入口按设备能力和辅助进程状态降级。
- 点击图标查看状态详情。
- 独立设置面板。
- 切换下方四个健康指示点的指标。
...
| 合并控制版本 | Wi-Fi 控制、控制状态确认、电源能力模型和可选辅助进程合同 |
...
应用只读取和展示当前 Mac 的本机状态；控制操作也只作用于当前 Mac。Wi-Fi 密码不持久化，“记住此网络”默认关闭且由用户主动选择。主应用不以 root 运行，不保存管理员密码或临时授权凭证；辅助进程仅在明确签名和授权后参与固定的电源控制合同。
